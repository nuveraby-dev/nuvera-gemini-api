...
# Добавление простого корневого маршрута для проверки живости API (устранение 404)
@app.route('/', methods=['GET'])
def home():
    return jsonify({"status": "ok", "service": "Nuvera Gemini API"}), 200
...
# На Vercel этот блок не запускается, Vercel напрямую ищет WSGI приложение 'app'
